...
export class UserRepository extends User {
 ...
 async update(args: UserUpdateArgs, context: Context): Promise<boolean> {
   const updated = await context.db('users')
   .where('id', this.id)
   .whereNull('deleted_at')
   .update({
     ...args,
     password_hash: args.password ? await context.password.encrypt(args.password) : this.password_hash,
 ...
 static async findByEmailOrPhone(email: string, phone: string, context: Context): Promise<User | null> {
