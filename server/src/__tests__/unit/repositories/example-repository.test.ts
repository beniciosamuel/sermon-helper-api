...
class ExampleRepositoryImpl implements ExampleRepository {
  ...
  async create(data: any): Promise<any> {
    const result = await this.db.query('INSERT INTO example (name, value) VALUES (?, ?)', [
      data.name,
      data.value,
    ]);
  ...
  async update(id: number, data: any): Promise<any> {
    await this.db.query('UPDATE example SET name = ?, value = ? WHERE id = ?', [
      data.name,
      data.value,
      id,
    ]);
...
describe('ExampleRepository', () => {
  ...
  describe('findById', () => {
    it('should return data when found', async () => {
      ...
      expect(mockDb.query).toHaveBeenCalledWith('SELECT * FROM example WHERE id = ?', [1]);
    });
    ...
    it('should return null when not found', async () => {
      ...
      expect(mockDb.query).toHaveBeenCalledWith('SELECT * FROM example WHERE id = ?', [999]);
    });
  });
  ...
  describe('create', () => {
    it('should create and return new data with id', async () => {
      ...
      expect(mockDb.query).toHaveBeenCalledWith('INSERT INTO example (name, value) VALUES (?, ?)', [
        inputData.name,
        inputData.value,
      ]);
    });
  ...
  describe('delete', () => {
    it('should return true when deletion succeeds', async () => {
      ...
      expect(mockDb.query).toHaveBeenCalledWith('DELETE FROM example WHERE id = ?', [1]);
    });
